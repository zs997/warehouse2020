...
//                            }
                            ...
                            if (lastState.getTargetValue() + col.getCost() < newState.getTargetValue()) {
                                ...
                                newState.setNumB(mb);
                                newState.setNumC(mc);
                                newState.setLastA(o);//保留最优情况下 上一个状态信息
                                newState.setLastB(p);
                                newState.setLastC(q);
                                newState.setEnterProb(col.getEnterProb());
                                newState.setEvenProb(col.getEvenProb());
...
//                }
    ...
    public static void check(Answer answer){
        ...
        for (int i = 0; i < columnAnswers.size(); i++) {
            ...
            usedB += numB;
            usedC += numC;
            col.calculCost(rowNO,usedA,usedB,usedC,numA,numB,numC,lastEvenProb,lastEnterProb);
            lastEnterProb = col.getEnterProb();
            lastEvenProb = col.getEvenProb();
